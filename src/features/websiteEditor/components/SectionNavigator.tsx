...
function SortableSection(
  ...
  return (
    ...
    >
      <div
        className={`group flex min-w-0 items-center gap-0.5 rounded-lg border px-1 py-1 ${props.selectedId === section.id && !selectedReference ? "border-accent border-2 bg-surface-muted" : "border-transparent hover:bg-surface-muted"} ${section.isEnabled ? "" : "bg-surface-muted/60 text-foreground-muted"}`}
      >
        <IconButton
          type="button"
          size="sm"
        ...
        <button
          ...
          aria-current={
            props.selectedId === section.id && !selectedReference
              ? "true"
              : undefined
          }
        >
