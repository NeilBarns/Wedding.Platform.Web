...
export function WebsiteColorSwatchControl({
  ...
  const choices = websiteColorChoices(allowedTemplateColorIds, templateColors, projectColors).map((choice) =>
    choice.kind === "inherit" ? { ...choice, label: inheritLabel } : choice,
  );
  ...
  const add = async () => {
    ...
    setError(null);
    try {
      const color = await onAddColor(pickerValue);
      onChange(color.id);
      close();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Unable to add this color.");
    } finally {
      setAdding(false);
    }
  };
  ...
  return <div ref={rootRef} className="relative min-w-0">
    ...
    {open && <div role="dialog" aria-label="Add Website color" className="absolute right-0 z-50 mt-2 w-[min(20rem,calc(100vw-3rem))] rounded-lg border border-border bg-surface p-3 shadow-xl">
      <ColorPicker id={`website-color-${pickerId}`} label="New Website color" value={pickerValue} onChange={setPickerValue} />
