...
import { ArrowDown, ArrowUp, ChevronDown, Copy, GripVertical, Plus, Trash2, Ungroup } from "lucide-react";
...
import { createDividerElement, createGroupElement, createRichTextElement, createTextElement, duplicateWebsiteElement, moveSectionChild, reorderSectionChild, ungroupSectionElement, updateGroupChildren, type SectionChildFlow, type SectionChildReference } from "../sectionChildFlow";
...
const elementLabel = (element: WebsiteElement) => element.type === "compositionGroup" ? "Group" : element.type === "text" ? element.text.trim() || "Text" : element.type === "richText" ? richTextPlainText(element.document) || "Rich Text" : element.type === "divider" ? "Divider" : element.type;
...
function TopLevelRow({ reference, index, sectionLabel, flow, selected, selectedElementId, onSelect, onChange, onDuplicate, onDelete }: { reference: SectionChildReference; index: number; sectionLabel: string; flow: SectionChildFlow; selected: boolean; selectedElementId?: string; onSelect: (reference: SectionChildReference) => void; onChange: (flow: SectionChildFlow) => void; onDuplicate: (id: string) => void; onDelete: (id: string) => void }) {
  ...
  return <div><div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={`relative flex min-w-0 items-center gap-0.5 rounded-md border px-1 py-0.5 ${selected ? "border-accent bg-surface-muted" : "border-transparent hover:bg-surface-muted"} ${isDragging ? "z-20 opacity-70 shadow-lg" : ""}`}>
    <IconButton type="button" size="sm" className="touch-none cursor-grab active:cursor-grabbing" aria-label={`Drag ${label}`} {...attributes} {...listeners}><GripVertical size={15} /></IconButton>
    <button type="button" className="min-w-0 flex-1 rounded px-1 py-1.5 text-left" aria-current={selected ? "true" : undefined} onClick={() => onSelect(reference)}><span className="block truncate text-xs font-medium">{label}</span></button>
...
function NestedRow({ child, sortableId: childSortableId, index, group, depth, flow, selectedElementId, onSelect, onChange }: { child: WebsiteElement; sortableId: string; index: number; group: CompositionGroup; depth: number; flow: SectionChildFlow; selectedElementId?: string; onSelect: (reference: SectionChildReference) => void; onChange: (flow: SectionChildFlow) => void }) {
  ...
  return <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={isDragging ? "relative z-20 opacity-70 shadow-lg" : ""}><div className={`flex min-w-0 items-center gap-0.5 rounded-md border px-1 py-0.5 ${selectedElementId === child.id ? "border-accent bg-surface-muted" : "border-transparent hover:bg-surface-muted"}`}>
    <IconButton type="button" size="sm" className="touch-none cursor-grab active:cursor-grabbing" aria-label={`Drag ${label}`} {...attributes} {...listeners}><GripVertical size={15} /></IconButton>
    <button type="button" className="min-w-0 flex-1 rounded px-1 py-1.5 text-left" aria-current={selectedElementId === child.id ? "true" : undefined} onClick={() => onSelect({ kind: "element", id: child.id })}><span className="block truncate text-xs font-medium">{label}</span></button>
