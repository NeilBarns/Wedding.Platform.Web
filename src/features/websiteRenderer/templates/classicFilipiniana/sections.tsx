...
export function ClassicFilipinianaStoryHeader({ sectionId, content, mode }: { sectionId: string; content: StoryContent; mode: 'editor' | 'public' }) {
  return <ContentSection botanical eyebrow="Our journey" heading={<EditableText sectionId={sectionId} path={['heading']} value={content.heading} fallback="Our Story" placeholder="Add heading" label="Story heading" />}>
    {content.intro?.trim() || mode === 'editor' ? <p className="mx-auto max-w-2xl whitespace-pre-line leading-8 text-[var(--cf-section-body)]"><EditableText sectionId={sectionId} path={['intro']} value={content.intro ?? ''} placeholder="Add introduction" label="Story introduction" multiline /></p> : null}
...
export function ClassicFilipinianaStoryBlockBody({ sectionId, block, index }: { sectionId: string; block: StoryBlock; index: number }) {
  return <p data-section-body className="mx-auto max-w-2xl whitespace-pre-line text-sm leading-8 text-[var(--cf-section-body)]"><EditableText sectionId={sectionId} path={['elements', index, 'body']} value={block.body} placeholder="Add story" label={`Story block ${index + 1} body`} multiline /></p>
}

export function ClassicFilipinianaSchedule({ sectionId, content }: { sectionId: string; content: ScheduleContent }) {
  return <ContentSection eyebrow="The celebration" heading={<EditableText sectionId={sectionId} path={['heading']} value={content.heading} fallback="Schedule" placeholder="Add heading" label="Schedule heading" />}>{content.items.length ? <ol className="mx-auto max-w-xl text-left">{content.items.map((item, index) => <li className="grid grid-cols-[6rem_1fr] gap-4 border-t border-[color-mix(in_srgb,var(--cf-border)_25%,transparent)] py-5" key={index}><span className="text-xs font-semibold uppercase tracking-wider text-[var(--cf-section-accent)]"><EditableText sectionId={sectionId} path={['items', index, 'time']} value={item.time} fallback="Time TBA" placeholder="Add time" label={`Schedule item ${index + 1} time`} /></span><div><h3 className="font-[family-name:var(--cf-heading-font)] text-xl text-[var(--cf-text)]"><EditableText sectionId={sectionId} path={['items', index, 'title']} value={item.title} fallback="Celebration detail" placeholder="Add title" label={`Schedule item ${index + 1} title`} /></h3><p className="mt-1 text-sm leading-6"><EditableText sectionId={sectionId} path={['items', index, 'description']} value={item.description} placeholder="Add description" label={`Schedule item ${index + 1} description`} multiline /></p></div></li>)}</ol> : <EmptyCopy>Celebration details will appear here.</EmptyCopy>}</ContentSection>
}
