...
import { WEBSITE_PROJECT_TEMPLATES, websiteTemplateDisplayName } from '../../features/websiteProjects/templates'
...
export function WebsiteProjectsPage() {
  ...
  const [templateKey, setTemplateKey] = useState<string>(WEBSITE_PROJECT_TEMPLATES[0].key)
  ...
  async function create() {
    ...
    try {
      const draft = await createWebsiteProject(event.id, { name: trimmed, templateKey })
      setOpen(false)
  ...
  return <WorkspaceSection eyebrow="Event websites" title="Website Projects" description="Create and manage independent Website drafts for this Event.">
    ...
    <Dialog open={open} onClose={() => !creating && setOpen(false)} closeDisabled={creating} titleId="create-website-title" descriptionId="create-website-description">
      <form className="p-5 sm:p-6" onSubmit={(formEvent) => { formEvent.preventDefault(); void create() }}>
        <DialogHeader title="Create Website" titleId="create-website-title" description="Choose a name and starting Template. The Template is selected for this new project." descriptionId="create-website-description" onClose={() => setOpen(false)} closeDisabled={creating} />
        <div className="mt-5 space-y-4"><label className="block text-sm font-medium" htmlFor="project-name">Project Name</label><Input id="project-name" value={name} maxLength={100} required disabled={creating} onChange={(event) => setName(event.target.value)} /><label className="block text-sm font-medium" htmlFor="project-template">Template</label><Select id="project-template" value={templateKey} disabled={creating} options={WEBSITE_PROJECT_TEMPLATES.map((template) => ({ value: template.key, label: template.displayName }))} onChange={setTemplateKey} />{formError && <Text variant="error" role="alert">{formError}</Text>}</div>
        <DialogFooter className="mt-6 border-t border-border pt-4"><Button variant="secondary" type="button" disabled={creating} onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={creating || !name.trim() || name.trim().length > 100}>{creating ? 'Creating…' : 'Create Website'}</Button></DialogFooter>
      </form>
    </Dialog>
  </WorkspaceSection>
}
